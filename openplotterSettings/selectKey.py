...
class SelectKey(wx.Dialog):
	def __init__(self, oldkey, selectvessels):
		...
		platform = Platform()
		self.port = platform.skPort
		self.http = platform.http
		...
		if oldkey:
			vessel = ''
			if oldkey[0:5] == 'self.' or oldkey[0:12] == 'urn:mrn:imo:' or oldkey[0:16] == 'urn:mrn:signalk:':
				items = oldkey.split('.')
				vessel = items[0]
				del items[0]
				oldkey = '.'.join(items)
			...
			if vessel: self.skvessels.SetValue(vessel)
